    ...
    >
      <Card className="w-full max-w-md fade-in">
        <CardHeader className="text-center">
          ...
          >
            {settings?.welcome_message || settings?.name || 'Leave a Review'}
          </CardTitle>
          <CardDescription>
            {settings?.description || 'Share your experience with us'}
          </CardDescription>
        </CardHeader>
        <CardContent>
...
export async function generateMetadata() {
  ...
  return {
    title: settings?.name ? `Review ${settings.name}` : 'Leave a Review',
